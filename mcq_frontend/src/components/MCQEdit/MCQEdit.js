...
const MCQEdit = ({ mcq, handleCancel, handleSave, isEditID, handleClickEdit, handleClickDelete, chooseAndAsk, handleChooseAndAsk }) => {
    ...
    return (
        ...
        <div className={cx('mcq-container', { ['mcq-choose']: chooseAndAsk === true, ["mcq-container-edit"]: isEditID === mcq._id })} >
                    ...
                    {editOptions.map((option, index) => {
                        return (
                            <li key={index} className='option-container fade'>
                                ...
                                >
                                    <i className="fa fa-trash fade" aria-hidden="true" onClick={() => { handleDeleteOption(index); }} style={{ fontSize: '15px', display: 'inline', paddingLeft: '15px', m: '30px', cursor: 'pointer', width: '100px' }}></i>
                                </CSSTransition>
